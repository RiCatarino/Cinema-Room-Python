...
letras = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J",
          "K"]  # Lista de letras para inserir no printa da sala, e receber os inputs dos utilizadores relativamente aos bilhetes
...
def mostrar_sala():
    ...
    for i in range(0,
                   11):  # Para cada linha inteira preenche o que está no respetivo indíce, seja um lugar vazio ou reservado
        ...
        print("|", letras[i], "\033[0m", sala[i][0], sala[i][1], "  ", sala[i][2], sala[i][3],
              ...
              sala[i][10], sala[i][11], "  ", sala[i][12], sala[i][13], "\033[94m", letras[i],
              "|")  # Print dos indíces com as diferentes cores em cada parte, e também as barras de lado
    print("\n\t   |----------------------------------------------------------|")
    print("\t   |\t\t\t\t\t\t\t      |")
    print("\t   |\t\t\t PALCO \t\t\t\t      |")
    print("\t   |\t\t\t\t\t\t\t      |")
...
def print_no_users_blocked():
    print("\033[91m\033[1m------ Não existem utilizadores bloqueados ------\033[0m\n")
