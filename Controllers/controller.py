...
def check_user_exists(username):
    cur.execute(
        f"SELECT * FROM Users WHERE username='{username}'")  # Query para percorrer por todos os registos o utilizador com aquele username
...
def checkUserLogin(username, password):
    cur.execute(
        f"SELECT * FROM Users WHERE username='{username}' AND password='{password}'")  # Query para percorrer por todos os registos as credenciais inseridas
    if cur.fetchone() != None:  # Se a query devolver algum valor, quer dizer que existe e devolve true
        return True
    else:
        return False
...
def verificar_data_duplicada(name, date):
    cur.execute(
        f"SELECT * FROM Datas_espetaculo WHERE data='{DateTime(date)}' AND espetaculo='{name}'")  # Query para verificar se já existe a data para o respetivo espetaculo
    result = cur.fetchone()
    return result
...
def check_letra(input):  # Verificar se o input tem alguma das letras dentro da lista "letras"
    ...
    if len(input) > 3:  # Se o tamanho do input for maior que 3 ou seja "ABCD" devolve falso
        return False
    if int(input[1:3]) < 1 or int(input[
                                  1:3]) > 14:  # SE O número do lugar for menor que 1 ou maior que 14 devolve falso, pois só pode ir de 1 a 14 conforme a sala
        return False
    return True
...
def convert_letters_in_numbers(lista, letras):  # CONVERTE AS LETRAS EM NÚMEROS
    for i in range(0, len(lista)):  # CICLO para percorrer a lista de reservas
        for k in range(0, len(letras)):  # CICLO para percorrer a lista de letras
            if lista[i][:1] == letras[
                k]:  # Ao percorrer a lista de letras, quando encontra a letra associada ao valor do indice da lista, por exemplo (K1)
                lista[i] = lista[i].replace(str(letras[k]), str(letras.index(letras[
                                                                                 k])) + " ")  # Converte o "K", neste caso em "0", pois substitui a letra na string pelo indíce onde está o "K", dentro da lista de letras.
    ...
    for item in range(0, len(lista)):  # por cada item dentro da lista de reservas
        list_to_print.append(
            str(lista[item]).split(" "))  # Adiciona o item e separa com espaço "0 1", "0 2", em vez de "01" ou "02".
    for item in range(0, len(list_to_print)):  # Ciclo para percorrer a lista a enviar para a sala
        try:
            view.sala[int(list_to_print[item][0])][int(list_to_print[item][
                                                           1]) - 1] = "\033[91m x \033[0m"  # Muda o lugar vazio para "x", conforme o respetivo indíce. (0, 1) (0, 2)
        ...
        if check_user_exists(username):
            ...
            while True:
                if password == repeatpassword:  # Enquanto as passwords forem diferentes, solicita a confirmação da password
                    mod.inserir_utilizador(username, password,
                                           role)  # Chama a função no model para inserir o utilizador, role e password na base de dados
    ...
    while True:
        ...
        if len(nome) > 0:
            if not check_espetaculo_exists(nome):
                mod.inserir_espetaculo(
                    nome)  # chama a função no model para inserir o espetaculo na tabela "Espetaculos"
                while True:
...
def listar_espetaculos(readonly):
    ...
    for espetaculo in cur.execute(
            "SELECT nome FROM Espetaculos"):  # Query para solicitar o nome de todos os espetaculos
        i += 1
        espetaculos.append(espetaculo[0])  # Insere na lista de espetaculos o valor devolvido
        view.print_lista_espetaculos(i,
                                     espetaculo)  # dá print ao espetaculo devolvido, juntamente com o indíce atual do ciclo
    view.print_line()
...
def listar_datas_espetaculo_para_reserva(espetaculo):
    ...
    for data in cur.execute(
            f"SELECT id, data FROM Datas_espetaculo WHERE espetaculo='{espetaculo}'"):  # Query para devolver as datas do respetivo espetaculo
...
def inserir_nova_data(nome_espetaculo):
    ...
    if verificar_data_duplicada(nome_espetaculo,
                                novadata):  # Verifica se a data já existe, se sim dá print e chama a função outra vez
        view.print_data_duplicada()
        inserir_nova_data(nome_espetaculo)
    else:
        mod.inserir_nova_data(nome_espetaculo,
                              novadata)  # Chama a função no mod para inserir a nova data na base de dados
...
def listar_todos_os_utilizadores():
    ...
    for user in cur.execute(
            "SELECT username, blocked FROM Users"):  # Query para selecionar todos os utilizador com o role "user"
...
def listar_utilizadores_ativos():  # Lista os utilizadores
    view.print_cabecalho_lista_users()
    for user in cur.execute(
            "SELECT username, blocked FROM Users WHERE role='User' AND blocked is NULL"):  # Query para selecionar todos os utilizador com o role "user"
        view.print_users(user)
        view.print_line()
...
def listar_utilizadores_bloqueados():
    view.print_cabecalho_lista_users()
    users_blocked = cur.execute("SELECT username, blocked FROM Users WHERE role='User' AND blocked = 'True'").fetchall()
    if users_blocked is None:  # Se não existirem users bloqueados, retorna False
...
def lugares_sala_reservados(
        data_espetaculo):  # Devolve todos os lugares reservados naquela data(id), cada data tem um id agregado ao filme
    cur.execute(
...
def escolha_quantidade(
        lugares_possiveis):  # Solicita a quantidade de bilhetes, se for menor que 0, inválido ou maior que o número de lugares disponíveis, dá os respetivos prints
    while True:
        try:
...
def reservar_bilhetes(espetaculo, data_espetaculo):
    ...
    lugares_reservados = lugares_sala_reservados(
        data_espetaculo)  # Pede à BD todos os lugares reservados naquela data (id)
    lugares_disponiveis = [x for x in lugares if
                           x not in lugares_reservados]  # Cria uma lista (lugares_disponiveis), com todos os lugares que não são "x"
    if lugares_disponiveis:  # Se houverem lugares disponiveis(a variável lugares_disponiveis não está vazia)
        convert_letters_in_numbers(lugares_reservados,
                                   view.letras)  # Converte as letras em números para preencher a sala
        view.mostrar_sala()
...
def escolha_auto(quantidade):
    ...
    if quantidade == 2:  # SE FOREM 2 RESERVAS, PROCURA NOS LUGARES DA ESQUERDA E DA DIREITA, PARA FICAREM JUNTOS
        for i in range(0, 11):  # PROCURA NA FILA DA ESQUERDA
            if view.sala[i][0] == " ▢ " and view.sala[i][
                1] == " ▢ ":  # SE ESTIVEREM OS DOIS LUGARES VAZIOS, INSERE OS DOIS NA LISTA DA NOVA RESERVA
                novareserva.append(f"{view.letras[i]}{1}")
                novareserva.append(f"{view.letras[i]}{2}")
                return novareserva
        for i in range(0, 11):
            if view.sala[i][12] == " ▢ " and view.sala[i][
                13] == " ▢ ":  # SE ESTIVEREM OS DOIS LUGARES VAZIOS, INSERE OS DOIS NA LISTA DA NOVA RESERVA
        ...
        if quantidade - len(novareserva) > 1:  # SE FOR MAIS QUE 1 BILHETE A FALTAR
            ...
            if all >= quantidade or all == 10:  # SE ALL for maior ou igual à quantidade ou for 10(máximo no meio)
                for k in range(2, 12):  # Ciclo para adicionar os lugares todos a linha atual
                    if (view.sala[i][
                            k] == " ▢ ") and f"sala[i]{k + 1}" not in novareserva:  # Se já não estiverem na lista da novareserva
                        novareserva.append(f"{view.letras[i]}{k + 1}")  # adiciona à lista
                        if quantidade == len(
                                novareserva):  # se a quantidade solicitada por igual ao número de bilhetes na lista de reservas, devolve a lista de reservas
                            return novareserva
...
def bilheteira_por_espetaculo():
    ...
    total = 0
    for item in cur.execute(
            f"SELECT id FROM Datas_espetaculo WHERE espetaculo='{espetaculo}'"):  # SELECIONA AS DATAS DO RESPETIVO ESPETACULO
        datas.append(item[0])
    if datas:
        total = get_total_bilheteira(datas)  # Calcula os valores associados
...
def remover_espetaculo():
    ...
    for data in datas:
        cur.execute(
            f"DELETE FROM User_espetaculo_lugar WHERE data_espetaculo='{data[0]}'")  # ELIMNA AS RESERVAS DO ESPETACULO
    for data in datas:
...
def remover_sessao():  # Remover sessão(data)
    ...
    if id_data_espetaculo is not None:
        cur.execute(
            f"SELECT data FROM Datas_espetaculo WHERE id='{id_data_espetaculo}'")  # procura pela data com esse id
        data_espetaculo = cur.fetchone()[0]
        cur.execute(
            f"SELECT lugar FROM User_espetaculo_lugar WHERE data_espetaculo='{id_data_espetaculo}'")  # Procura pelas reservas nessa data
        ...
        if decisao == "SIM":
            cur.execute(
                f"DELETE FROM User_espetaculo_lugar WHERE data_espetaculo ='{id_data_espetaculo}'")  # ELIMINA AS RESERVAS NESSA DATA
